...
class PruningWeight(object):
    ...
    def _FindMidValue(self, weight, ratio):
        ...
        # flatten's the weight
        ft_weight = torch.flatten(torch.abs(weight))
        
        # sorting ft_weight valuees
        inorder_vals,_ = torch.sort(ft_weight)
        
        # extracting index values from ft_weight size
        indx_vals = int(ratio*ft_weight.size()[0])
        
        # setting ind_vals for sorted values
        threshold = inorder_vals[indx_vals]
    ...
    def _InitMask(self, w, threshold):
        ...
        # setting absolute values of torch for w involving parameter's
